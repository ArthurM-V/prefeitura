...
class AdminController {
    ...
    public function atribuirOrgao(): void {
        ...
        if ($chamadoModel->atribuirOrgao($chamadoId, $orgaoId)) {
            if ($chamado['status_id'] == 1) {
                $chamadoModel->alterarStatus($chamadoId, 2);
            }
            $historicoModel->registrar(
                'Atribuição',
                "Chamado atribuído ao órgão: {$orgao['nome']}.",
                $_SESSION['usuario_id'],
                $chamadoId
            );
    ...
    public function enviarFeedback(): void {
        ...
        if (!$chamadoId || !$mensagem) {
            jsonResponse(['success' => false, 'message' => 'Mensagem não pode ser vazia.'], 422);
        }
        ...
        if ($feedbackModel->criar($mensagem, $_SESSION['usuario_id'], $chamadoId)) {
            ...
            $historicoModel->registrar(
                'Resolução',
                'Feedback enviado ao cidadão e chamado marcado como resolvido.',
                $_SESSION['usuario_id'],
                $chamadoId
            );
            jsonResponse(['success' => true, 'message' => 'Feedback enviado e chamado marcado como resolvido.']);
        }
